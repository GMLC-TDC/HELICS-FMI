    ...
    if ((ext == ".fmu") || (ext == ".FMU")) {
        try {
            ...
            if (cosimFmu && fmi.checkFlag(fmuCapabilityFlags::coSimulationCapable)) {
                ...
                auto fed = std::make_unique<CoSimFederate>("", std::move(obj),core->getCopyofCorePointer(), fedInfo);
...
int FmiRunner::loadFile(readerElement& elem)
{
    ...
    while (elem.isValid()) {
        ...
        if (fmilib->checkFlag(fmuCapabilityFlags::coSimulationCapable)) {
            ...
            auto fed = std::make_unique<CoSimFederate>(obj->getName(), std::move(obj),core->getCopyofCorePointer(), fedInfo);
