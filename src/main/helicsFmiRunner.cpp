...
namespace helicsfmi
{
    ...
    std::unique_ptr<CLI::App> FmiRunner::generateCLI()
    {
        ...
        app->final_callback([this,apt=app.get()](){auto remArgs = apt->remaining_for_passthrough();
        ...
        app->footer([]() { [[maybe_unused]] const helics::FederateInfo fedInfo("--help"); return std::string{}; });
        return app;
    }
    ...
    int FmiRunner::load()
    {
        if (fedInfo.autobroker) {
            try {
                broker =
                    std::make_unique<helics::apps::BrokerApp>(fedInfo.coreType, brokerArgs);
