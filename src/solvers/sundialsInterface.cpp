...
#define CHECK_JACOBIAN 0
                    ...
                    N_Vector /*tmp2*/)
    {
        ...
        if (MatrixNeedsSetup(sd->jacCallCount, J)) {
            ...
            if (sd->flags[fileCapture_flag]) {
                if (!sd->jacFile.empty()) {
                    //    writeArray (time, 1, sd->jacCallCount, sd->mode.offsetIndex, *a1,
                    //    sd->jacFile);
                }
            }
        }
...
sd->sobj->jacobianFunction (time, NVECTOR_DATA(sd->use_omp, state), NVECTOR_DATA(sd->use_omp,
dstate_dt), a1,cj, sd->mode); a1.sortIndexCol (); if (sd->flags[useMask_flag])
{
for (auto &v : sd->maskElements)
{
