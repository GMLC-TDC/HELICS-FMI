...
internal operations require a swap, however in high contention the two locks will reduce contention
in most cases.
*/
template<typename T>
class BlockingQueue {
  ...
  public:
    ...
    ~BlockingQueue()
    {
        // these locks are primarily for memory synchronization multiple access in the destructor
