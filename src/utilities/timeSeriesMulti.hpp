...
// don't need the time series object but if you include this I want timeSeries to be included as
// well along with all the other objects
...
class timeSeriesMulti {
  ...
  private:
    ...
    std::vector<std::string>
        fields;  //!< container for all the strings associated with the different columns
  ...
  public:
    ...
    */
    ...
    {
        for (auto& dc : m_data) {
            std::transform(dc.begin(), dc.end(), dc.begin(), [factor](dataType val) {
                return val * factor;
            });
        }
    }
    ...
    */
    ...
    void loadBinaryFile(const std::string& fileName)
    {
        ...
        setCols(
            rcount -
            1);  // update the number of columns the file contains the time, then the m_data columns
