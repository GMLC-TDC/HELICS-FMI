...
void workQueue::addWorkBlock(std::vector<std::shared_ptr<basicWorkBlock>>& newWork,
...
{
    if (!pInstance) {
        // this is to ensure we don't create it multiple times resulting in some weird condition or
...
void workQueue::workerLoop()
{
    ...
    while (!halt) {
        ...
        auto wb = getWorkBlock();  // this will return empty if it is spurious and also sync the
                                   // size if needed
        if ((wb) && (!wb->isFinished())) {
            wb->execute();
