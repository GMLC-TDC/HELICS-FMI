...
class timeSeries {
  ...
  public:
    ...
    */
    ...
    @param[in] append  flag indicating that if the file exists it should be appended rather than
    overwritten
    */
    ...
    @param[in] fileName  the file to write
    @param[in] precision  the number of digits that should be included for non-integer data in the
    file
    @param[in] append  flag indicating that if the file exists it should be appended rather than
    overwritten
    */
    void writeTextFile(const std::string& fileName, int precision = 8, bool append = false)
    {
        ...
        for (size_t rr = 0; rr < count; rr++) {
            fio << std::setprecision(5) << m_time[rr] << ',' << std::setprecision(precision)
                << m_data[rr] << '\n';
        }
        fio.close();
    }
