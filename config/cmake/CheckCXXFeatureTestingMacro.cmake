...
macro(CHECK_CXX_FEATURE_TESTING_MACRO SYMBOL FILES VARIABLE)
    ...
    if(CMAKE_CXX_COMPILER_LOADED)
        _check_preprocessor_macro(
            "${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeTmp/CheckFeatureTestingMacro.cxx"
            "${SYMBOL}" "${FILES}" "${VARIABLE}" "${EXPECTED_VALUE}"
        )
    else()
...
macro(
    ...
    if(NOT DEFINED "${VARIABLE}" OR "x${${VARIABLE}}" STREQUAL "x${VARIABLE}")
        ...
        configure_file("${CMAKE_ROOT}/Modules/CMakeConfigurableFile.in" "${SOURCEFILE}" @ONLY)

        if(NOT CMAKE_REQUIRED_QUIET)
            message(STATUS "Looking for ${SYMBOL}")
        endif()
        try_run(
            ${VARIABLE}_EXITCODE ${VARIABLE}_COMPILED ${CMAKE_BINARY_DIR} "${SOURCEFILE}"
            COMPILE_DEFINITIONS ${CMAKE_REQUIRED_DEFINITIONS} ${CHECK_SYMBOL_EXISTS_LIBS}
            CMAKE_FLAGS
                -DCOMPILE_DEFINITIONS:STRING=${MACRO_CHECK_SYMBOL_EXISTS_FLAGS}
                -DCMAKE_SKIP_RPATH:BOOL=${CMAKE_SKIP_RPATH} "${CMAKE_SYMBOL_EXISTS_INCLUDES}"
        ...
        if(${VARIABLE}_COMPILED)
            if("${${VARIABLE}_EXITCODE}" EQUAL 0)
                ...
                file(APPEND ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeOutput.log
                     "Determining if the ${SYMBOL} " "exist passed with the following output:\n"
                     "${OUTPUT}\nFile ${SOURCEFILE}:\n" "${CMAKE_CONFIGURABLE_FILE_CONTENT}\n"
                )
            else()
                if(NOT CMAKE_REQUIRED_QUIET)
                ...
                file(APPEND ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeError.log
                     "Determining if the ${SYMBOL} " "exist failed with the following output:\n"
                     "${OUTPUT}\nFile ${SOURCEFILE}:\n" "${CMAKE_CONFIGURABLE_FILE_CONTENT}\n"
                )
            endif()
        else()
